...
`Handler`, and finalizes the backend message with the handler's explicit `AckDecision`.
...
supervision, ordering policy, concurrency policy, graceful shutdown, and trace spans around handler
execution.
...
Message DB adapters documented in the integration section.
...
adds the backend-provided `AckHandle` and an optional `Lease` for queues with visibility timeouts.
...
An **envelope** is the normalized message container inside `Ingested`. It is where adapter metadata
