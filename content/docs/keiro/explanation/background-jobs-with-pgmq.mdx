...
- write a handler of type `Message es Value -> Eff es AckDecision` that **decodes JSON by hand** and
...
data Job p = Job
  ...
  , jobOrdering :: !JobOrdering -- required read/ordering contract
...
The same `Job` API drives two genuinely different run shapes:

| | `runJobWorkers` (continuous) | `runJobOnce` (one-shot drain) |
|---|---|---|
| Shape | Supervise N processors until caller-owned shutdown | Drain up to `n` messages, then stop |
...
`runJobWorkers` is for a long-lived process; it builds every processor with
...
The ordering vocabulary is `unordered`, `fifo-throughput`, `fifo-roundrobin`, and `fifo-heads`.
`fifo-heads` is the strict PGMQ 1.12 strategy and safely supports batches; the two legacy FIFO
strategies are restricted to singleton reads. FIFO is
