...
<Callout type="warn">
A `PMCommandFailed` is classified by the worker: transient store/conflict failures retry after
`transientRetryDelay`, while deterministic command errors halt. Model benign domain rejections (a
target that declines a command because no edge matches in its current state) as **total** transitions
in the keiki transducer so they never surface as a deterministic failure. See [Keep target commands
