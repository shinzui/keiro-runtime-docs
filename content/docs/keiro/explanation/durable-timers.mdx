...
stateDiagram-v2
  ...
  Firing --> Dead: deadLetterTimer / maxAttempts ceiling / expired resume lease
  Scheduled --> Dead: deadLetterTimer
  Dead --> Firing: claimDeadTimer (exact guards + lease)
  Firing --> Dead: parkTimerResume
  Fired --> [*]
  Cancelled --> [*]
  Dead --> [*]
```
...
- **`Cancelled`** — withdrawn before firing via `cancelTimer`; terminal. `Cancelled` is **also the
  ...
  to `Cancelled`).
- **`Dead`** — parked or abandoned after exceeding the attempt ceiling, via `deadLetterTimer` (or
  the worker's `maxAttempts` auto-dead-letter). Carries a `last_error`; Keiro 0.16 can resume it only
  through an exact, token-guarded foreground claim.
...
option to `Nothing` only when a separate recovery loop owns this ordinary transition. Expired
foreground claims still recover directly to `Dead` on every worker pass. Keiro also ships a
...
## Inspecting and resuming dead work

Keiro 0.16 separates **observation** from **ownership**. `lookupTimerInspection` and bounded
`findDeadTimers` pages expose the stored reason without claiming the row. After application-level
authorization and session preflight, `claimDeadTimer` atomically checks the exact timer id, owner,
reason, and total attempt ceiling, increments the attempt, and returns an opaque expiring claim.

Only that claim may renew, complete, park, or cancel the resumed work. Losing ownership returns
`False`; the caller must stop where possible and must not report success. ID-only recovery functions
refuse guarded rows even after lease expiry until `recoverExpiredTimerResumes` returns them to
`Dead`. The token prevents two cooperating writers from finalizing each other's work, but it cannot
make an external API exactly-once—keep a durable application receipt.

Migration `0032` adds the paired token/lease columns. Its rollout must drain old timer writers before
the migration and deploy every upgraded writer before enabling resume. See [Resume a dead timer
safely](/docs/keiro/how-to/resume-a-dead-timer-safely) for the complete runbook.
