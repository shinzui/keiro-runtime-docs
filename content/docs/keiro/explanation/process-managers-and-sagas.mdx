...
when the *next* action depends on a history the coordinator must remember.
...
## The sharp edge: a failed dispatch wedges the worker
...
`PMCommandFailed` is **fatal by design**. When a process manager runs as a live subscription
(`runProcessManagerWorker`), a reaction that returns `Left`, or that a higher-level loop treats as
failed, finalizes `AckHalt (HaltFatal …)` — so the source event is **retried forever**. That is
the correct behavior for a *transient* failure, but it is a trap for a *benign* rejection: if a
target aggregate rejects a command simply because it is already in the right state, and that
surfaces as `PMCommandFailed`, the worker wedges.
