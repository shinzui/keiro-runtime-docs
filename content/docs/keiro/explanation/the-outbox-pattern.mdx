...
A separate worker, `publishClaimedOutbox`, claims pending rows, hands the claimed batch to your
publish function, and marks each row sent only **after the broker acknowledges**.
...
the next publisher pass reclaims the stale `publishing` row and publishes it **again** —
at-least-once publish. That double-send is harmless precisely because the consuming
[inbox](/docs/keiro/explanation/the-inbox-pattern) deduplicates on the stable `messageId`. The two
patterns are designed as a pair: at-least-once publish + idempotent receive = exactly-once effect.
...
- **`PerKeyHeadOfLine`** (the default) — within a `source`, a non-terminal row with key `k` blocks
  every *later* row with the same key, preserving the outbox sequence per aggregate; rows with a `Nothing` key
...
These policies preserve the outbox's `(created_at, outbox_id)` sequence, not an independent notion of
commit order. PostgreSQL supplies `created_at` at transaction start. The canonical producer
subscription serializes same-key input; inline same-key/source enqueues from concurrent transactions
must be serialized by the application when strict order matters, because those transactions can
commit opposite their start timestamps.
...
Schedule `outboxMaintenancePass` separately from the publish hot path to reclaim stale `publishing`
rows. Rows below the attempt ceiling move back to `failed`; rows at or above the ceiling move to
`dead`. This closes the old crash-recovery gap where a mid-publish process death could block a key
forever without adding a backlog query to every publish pass.
...
two-phase commit. Tune the stale-publishing timeout conservatively so recovery is prompt without
mistaking a slow broker call for a crashed publisher.
