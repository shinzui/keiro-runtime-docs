...
Before each claim pass, `publishClaimedOutbox` reclaims stale `publishing` rows. Rows below the
attempt ceiling move back to `failed`; rows at or above the ceiling move to `dead`. This closes the
old crash-recovery gap where a mid-publish process death could block a key forever.
