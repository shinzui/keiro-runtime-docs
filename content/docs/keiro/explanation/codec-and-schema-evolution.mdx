...
```haskell
type Upcaster = (Int, Value -> Either Text Value)   -- (source version it upgrades FROM, migration)
```
...
version recorded on disk is authoritative — you cannot accidentally overwrite it.
</Callout>
...
1. `extractSchemaVersion` reads the stamped version off the `RecordedEvent`, **defaulting to `1`**
   when it is absent or invalid (so events written before stamping existed still decode).
...
allowed set. The upcaster chain must be **ascending and contiguous**: if migration reaches version
`n` but the next available upcaster starts later, you get `GapInUpcasterChain n nextVersion`. An
upcaster that returns `Left` becomes `UpcasterError`.
</Callout>
