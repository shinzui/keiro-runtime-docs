...
flowchart TD
  ...
  K --> I["INSERT completed inbox row ON CONFLICT DO NOTHING"]
  I -->|new row| H["run handler + local writes"]
  H --> OK["InboxProcessed a (row + handler commits atomically)"]
  I -->|row exists, completed| D["InboxDuplicate (handler NOT run)"]
  I -->|row exists, processing| P["InboxInProgress"]
  I -->|row exists, failed| F["InboxPreviouslyFailed"]
```

The completed inbox-row insert and the handler's local writes **commit together or not at all**. This
is the crux of the idempotency:
...
## A v1 detail: successful intake skips the visible processing row

The `InboxProcessing` status remains decodable for legacy rows and reserved async paths, but current
fresh successful intake inserts directly as `completed`; it no longer writes a `processing` row and
then updates it. So the normal happy path is `InboxProcessed` then `InboxDuplicate` on redelivery.
The retrying wrapper is the exception: it records `failed` rows explicitly for operator-visible
poison accounting.

```mermaid
flowchart LR
  Single["single-message wrapper"] --> Completed["fresh success: one completed row insert"]
  Batch["batch wrapper"] --> Shared["many messages in one transaction"]
  Shared --> Fallback["on throw/condemn: fall back to retrying singles"]
  Slim["PersistDedupeOnly"] --> Dedupe["success rows keep only dedupe/correlation data"]
  Full["PersistFullEnvelope"] --> Envelope["success rows keep full envelope"]
  Dedupe --> Failed["failed rows always keep full envelope"]
  Envelope --> Failed
```
