...
store and run a command). The domain names mirror the legacy
`Jitsurei.FulfillmentProcess` example, but the old package is not a current runnable release gate.
</Callout>
...
[getting-started tutorial](/docs/keiro/tutorials/getting-started) covers this), plus an equivalent
order aggregate in your tutorial project: `OrderCommand` / `OrderEvent` / `OrderState`,
`orderEventStream`, and `orderCommandStream` (see the [EventStream and Stream
reference](/docs/keiro/reference/event-stream-and-stream)). The names below preserve the legacy
example vocabulary so the process-manager relationships stay easy to follow.
...
command to a target aggregate, all replay-safe under at-least-once delivery. Next, learn how to run
the manager continuously in [Run a process manager as a
