...
<Callout type="info">
  *(Keiro 0.9.0.0.)* `Keiro.Inbox.Types` re-exports **`RetryDelay`** from `Shibuya.Core.Ack`, so a
  generated inbox disposition that carries a retry delay needs only the `keiro` import — no direct
  `shibuya-core` dependency for that name alone. The same release also closed the generated inbox
  surface over `Text`: generation now exposes **closed outcome and detailed disposition sums** rather
  than stringly-typed values, so re-scaffolding a spec will surface type errors where a `Text`
  disposition used to pass. See [runtime and integration
  nodes](/docs/keiro/reference/keiro-dsl-runtime-nodes).
</Callout>
...
## Delegated idempotence *(Keiro 0.17+)*

Delegated intake omits the `keiro_inbox` row and lets the downstream atomic operation own the durable
deduplication receipt. These runners require `IOE` but not `Store`; the handler receives the computed
dedupe key and must cover every protected effect with that identity.
...
`runInboxDelegatedBatch` is sequential. It suppresses a repeated identity within that call only after
the first occurrence succeeds or reports a durable duplicate. It creates no transactions or worker
threads.

The retrying variant takes a caller-owned, one-based delivery position. Attempts above the ceiling
return `InboxPreviouslyFailed` without invoking the handler; synchronous exceptions at or below it
become `InboxHandlerFailed`. Async cancellation and typed effect errors still propagate.

```haskell
data DelegatedRetryContext -- constructor is hidden
...
`Keiro.Inbox.Delegated` supplies the safer single-command adapters:
...
delegatedFromPMCommand ::
  StreamName -> PMCommandResult target ->
  Either DelegatedCommandError (DelegatedOutcome (CommandResult target))
```

`delegatedCommand` uses a deterministic first-event ID as the receipt, confirms duplicates in the
resolved target stream, and rejects a successful command that appended no event. The command and all
protected SQL, projection, and outbox work must be one atomic append transaction.

<Callout type="warn">
  Delegated intake has no inbox backlog, failed-row inspection, or inbox GC record. Do not use it for
  silent commands, non-atomic side effects, or an arbitrary multi-command reaction. Prefer the
  table-backed wrappers whenever the downstream operation cannot supply one complete durable receipt.
</Callout>
...
listCompletedInboxGcCandidates :: (Store :> es) => NominalDiffTime -> UTCTime -> Eff es [InboxRow]
```

`listCompletedInboxGcCandidates` is the read-only operator **preview** for the same retention window:
it lists the completed rows a pass would delete, oldest first, without deleting anything. It backs
[`keiro-ops inbox gc`](/docs/keiro/reference/keiro-ops-cli#inbox--integration-event-inbox), so an
operator surface never reproduces the eligibility query. A later `garbageCollectCompleted` re-evaluates
eligibility, so the preview is informational rather than a reservation. *New in Keiro 0.12.0.0.*
