---
title: Code Walkthrough
description: "Ordered tours through keiro's real source — one per subsystem."
---

These are **ordered source tours**: numbered chapters that read keiro's real Haskell source to
teach how each subsystem is built. Read each tour in order. There is one tour per subsystem, plus a
**foundation** tour for the keiki↔keiro core that the others build on and a cross-cutting
**operations** tour for telemetry and migrations.

<Cards>
  <Card title="The foundation" href="/docs/keiro/walkthrough/foundation/00-start-here" description="The keiki↔keiro core: the typed EventStream/Stream handles, the codec boundary, and the SymTransducer/step that threads (state, registers)." />
  <Card title="The command cycle" href="/docs/keiro/walkthrough/command-cycle/00-start-here" description="Hydrate → Transduce → Append: Keiro.Command, the codec, and the typed event stream." />
  <Card title="The read side" href="/docs/keiro/walkthrough/read-side/00-start-here" description="Event-folded projections and read models, followed by the separate write-side snapshot cache for keiki state and registers." />
  <Card title="The workflow engine" href="/docs/keiro/walkthrough/workflow/00-start-here" description="Process managers and durable timers." />
  <Card title="Durable execution" href="/docs/keiro/walkthrough/durable-execution/00-start-here" description="The Keiro.Workflow runtime: journal-and-replay over named steps, durable sleep, awakeables, child workflows, the resume worker, snapshots, and versioning/rotation." />
  <Card title="Scaling the workers" href="/docs/keiro/walkthrough/scaling/00-start-here" description="Push-aware polling plus leased, acknowledgment-coupled consumer groups with bounded retry and dead-letter recovery." />
  <Card title="Integration events" href="/docs/keiro/walkthrough/integration/00-start-here" description="The inbox, the outbox, and the Kafka bridge." />
  <Card title="Operations" href="/docs/keiro/walkthrough/operations/00-start-here" description="The cross-cutting internals: Keiro.Telemetry span helpers and the Keiro.Migrations runner." />
</Cards>
