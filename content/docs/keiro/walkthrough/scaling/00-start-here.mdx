...
- **Consumer-group sharding** (`Keiro.Subscription.Shard`) lets a pool of identical workers drain a busy
  category subscription disjointly, leasing kiroku consumer-group buckets with coordinator-free failover.
...
keiro/src/Keiro/Subscription/Shard/Worker.hs     -- reconcileShardsOnce, startReader, runShardedSubscriptionGroup
```
...
<Cards>
  ...
  <Card title="04 — The sharded worker" href="/docs/keiro/walkthrough/scaling/04-the-sharded-worker" description="reconcileShardsOnce (estimate live workers, acquire, shed excess, reconcile readers), startReader's consumer-group config, the finally-on-shutdown failover, and the rebalance-signal seam." />
</Cards>
...
(`subscriptionStream` with a `ConsumerGroup{member, size}`).
