...
This tour documents the process-manager and timer surface in the current `0.3.0.0` release,
including what is *not* there. So you are not surprised later:
...
retrying it forever. `defaultTimerWorkerOptions` also runs a bounded stale-row sweep before each pass,
requeueing `Firing` rows after five minutes; the timeout is configurable and must exceed the longest
valid fire action. See [chapter 04](/docs/keiro/walkthrough/workflow/04-the-timer-schema).

## The chapters

<Cards>
  <Card title="01 — The process manager: the dispatch loop" href="/docs/keiro/walkthrough/workflow/01-the-process-manager-dispatch-loop" description="Point-lookup idempotency, target-scoped duplicate proof, and target-bearing results." />
  <Card title="02 — The transaction and acknowledgement model" href="/docs/keiro/walkthrough/workflow/02-the-transaction-model" description="Separate commits, convergent redelivery, explicit rejection policy, and the two dead-letter meanings." />
  <Card title="03 — The ProcessManager types and config" href="/docs/keiro/walkthrough/workflow/03-the-types-and-config" description="ProcessManager, actions, target-bearing results, rejection options, and timer types." />
...
<Callout type="info">
**The benefit to watch for: a process manager's *own* state is a keiki `SymTransducer`** — the same
object an aggregate is. It is "just another aggregate", event-sourced to its own stream and stepped
through the *same* command cycle, so this engine re-implements no state storage, replay, or concurrency
of its own. That reuse — typed registers for a saga's working memory, ε-edges that let the manager
advance itself, and replay-safety inherited from the transducer's determinism — is highlighted in
[chapter 01](/docs/keiro/walkthrough/workflow/01-the-process-manager-dispatch-loop#the-payoff-the-managers-own-state-is-a-keiki-transducer)
and argued in full in [Why SymTransducer, not Decider](/docs/keiro/explanation/why-symtransducer-not-decider).
</Callout>

The current source contracts stand on their own in this tour; the separate example application is
not used as proof of the 0.2 API surface.
