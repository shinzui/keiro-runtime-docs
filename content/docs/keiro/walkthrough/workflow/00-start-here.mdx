...
This tour documents keiro **as shipped at version 0.1.0.0**, including what is *not* there. So
you are not surprised later:

- **No timer-cancellation SQL ships.** `Cancelled` exists only as a `TimerStatus` constructor
  and as the decode fallback for an unrecognized stored status. There is no `cancelTimer`
  function. See [chapter 04](/docs/keiro/walkthrough/workflow/04-the-timer-schema).
- **No backoff.** There is no `next_attempt_at` column and no exponential schedule; `attempts`
  is incremented but nothing delays a retry. A `Firing` row is immediately re-claimable.
- **No dead-letter / max-attempts.** A poison timer (or a `PMCommandFailed` dispatch) is
  retried indefinitely; the operator must watch `attempts`.
- **The timer worker is bare.** `runTimerWorker` has no loop, no clock, and no supervisor — the
  caller passes `now` and drives the tick. See
...
<Cards>
  ...
  <Card title="04 — The timer schema and claim SQL" href="/docs/keiro/walkthrough/workflow/04-the-timer-schema" description="The keiro_timers columns, the claim CTE with FOR UPDATE SKIP LOCKED, the re-arm ON CONFLICT guard, the decoder, and the honest gaps." />
