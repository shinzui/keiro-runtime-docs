...
<Cards>
  ...
  <Card title="03 — The migration runner" href="/docs/keiro/walkthrough/operations/03-the-migration-runner" description="embedDir, parseAddedSqlMigration, the kiroku-then-keiro order, the four runners, the keiro-migrate CLI (apply + the new scaffold), and the nine tables." />
</Cards>
...
keiro-migrations/app/Main.hs              -- the keiro-migrate CLI (apply + new)
keiro-migrations/src/Keiro/Migrations/New.hs -- the `keiro-migrate new` scaffold
keiro-migrations/sql-migrations/*.sql     -- the nine embedded files (nine tables)
```
