...
(a value whose tag is not in `eventTypes`). Decoding adds the four migration faults: `UnknownVersion`,
`UpcasterError`, `DecodeFailed`, and `GapInUpcasterChain`.
...
the stamp on disk is authoritative. `encodeForAppend` is just `encodeForAppendWithMetadata` with no
caller metadata.
...
1. **Reject unknown tags first.** If the recorded event's type tag is not in `eventTypes`, decoding
   ...
   guess.
2. **Read the stamped version back** via `extractSchemaVersion`, which defaults to `1` when the
   metadata is absent, is not an object, lacks the `schemaVersion` key, or holds a non-integer.
...
flowchart LR
  ...
  Known -- "yes" --> Ver["extractSchemaVersion (default 1)"]
