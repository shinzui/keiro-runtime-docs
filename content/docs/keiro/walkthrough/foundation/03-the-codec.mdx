...
<TypeTable
  type={{
    eventTypes: { description: "The complete set of event-type tags this codec owns. Encoding and decoding reject any tag outside this set, so it doubles as the stream's event-type allow-list.", type: "!(NonEmpty EventType)" },
    eventType: { description: "Projects a domain value to its wire tag (must land in eventTypes).", type: "!(e -> EventType)" },
...
`schemaVersion = 2` means new events are written at version 2; the single rung
`(1, const upcastOrderPlacedV1)` brings a stored version-1 payload up to version 2 before
`parseOrderEvent` runs. (`const` discards the rung's `EventType` argument, because this rung migrates
one kind; a generated multi-event rung dispatches on it instead.) The `eventTypes` `NonEmpty` is the
allow-list: a recorded event tagged with anything else is rejected on read.
