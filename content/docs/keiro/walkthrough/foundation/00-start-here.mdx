...
`(state, registers)` pair from stored events), **Transduce** (`Keiki.stepEither` produces a structured
decision or the next pair plus events), **Append** (encode via the codec and write):

```mermaid
flowchart LR
  Cmd["command (ci)"] --> H["Hydrate: rebuild (state, registers)"]
  H --> T{"Transduce: Keiki.stepEither (state, regs) ci"}
  T -- "no match" --> Rej["CommandRejected"]
  T -- "multiple matches" --> Amb["CommandAmbiguous"]
  T -- "Right (s', regs', events)" --> Enc["encode events via Codec"]
  Enc --> A["Append at expectedVersion"]
```

The `(state, registers)` **pair** is the thread running through every box: hydration rebuilds it,
`stepEither` consumes and returns it, and a snapshot persists it.
