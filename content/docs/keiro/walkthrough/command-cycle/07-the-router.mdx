    ...
    handleIngested Ingested{envelope = env@Envelope{payload = message}, ack = AckHandle finalizeAck} = do
      decision <- case decodeMessage message of
        ...
        Just (recorded, input) -> do
          ...
          pure (ackDecisionFor outcome)
      finalizeAck decision
      pure decision

    ackDecisionFor outcome =
      -- transient failures retry; deterministic failures halt; all appended/duplicate results ack OK
      ...
```

`runRouterWorker` runs the router as a live subscription, draining a shibuya `Adapter` and finalizing
each message's `AckHandle` with a decision. The ack policy is strict but classified: an undecodable
message follows `PoisonPolicy` (default halt); after dispatch, an
all-`PMCommandAppended`/`PMCommandDuplicate` batch acks `AckOk`; transient failures retry after
`transientRetryDelay`; deterministic failures halt.

<Callout type="warn">
Because a deterministic `PMCommandFailed` halts, a *benign* domain rejection (a target with no matching
edge) would wedge the worker. Model such rejections as **total** transitions (an ε-complement
self-loop) in the target transducer so they never become `PMCommandFailed`.
</Callout>
