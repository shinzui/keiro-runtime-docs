...
<Callout type="info">
  Current docs include the July 2026 lifecycle update: streams now have a reversible
  `truncateBefore` marker exposed by `setStreamTruncateBefore` /
  `clearStreamTruncateBefore`. It bounds direct per-stream rehydration without deleting events or
  changing `$all`, category reads, or subscriptions.
</Callout>
