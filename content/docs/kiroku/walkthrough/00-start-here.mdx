...
flowchart TD
  ...
  PG -->|"read $all once"| Pub
  Pub -->|"live reads (AllStreams)"| Worker
...
parts each take one box of this diagram.
...
- [03 — The event publisher](/docs/kiroku/walkthrough/03-the-event-publisher): the `$all` broadcaster that fills the live queue.
- [04 — subscribe & lifecycle](/docs/kiroku/walkthrough/04-subscribe-and-lifecycle): spawning the worker and the handle.
