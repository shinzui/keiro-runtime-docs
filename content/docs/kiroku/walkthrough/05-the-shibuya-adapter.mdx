...
flowchart TD
  ...
  LiveQ["AllStreams publisher TBQueue<br/>(Vector RecordedEvent)<br/>cap = queueCapacity (16)"]
  ...
  AckQ["ack TBQueue<br/>(AckItem)<br/>cap = bufferSize (256)"]
...
1. **Front half — push, driven by LISTEN/NOTIFY (part 02's territory).** A `NOTIFY` on append wakes
   ...
   registered non-group `AllStreams` subscriber's own bounded `TBQueue`. The worker reads its batch
   from that queue. (A `Category` or consumer-group worker registers no publisher queue and
   re-queries with a SQL filter when the publisher's position advances — same wakeup, different
   fetch. Catch-up always reads the database directly.)
...
kirokuAdapter store KirokuAdapterConfig
  { subscriptionName = subName, subscriptionTarget = subTarget
  , batchSize = bs, bufferSize = buf, queueCapacity = qCap
  , consumerGroup = cg, eventTypeFilter = etf, selector = sel
  } = do
  ...
  let subConfig =
        (defaultSubscriptionConfig subName subTarget (\_ -> pure Continue))
          { batchSize       = bs
          , queueCapacity   = qCap           -- the *front-half* per-subscriber queue (batches)
          -- overflowPolicy is inherited from defaultSubscriptionConfig: PauseAndResume
          , consumerGroup   = cg
          , eventTypeFilter = etf
          , selector        = sel
          }
...
2. There are **two** capacities, and they are different queues. `queueCapacity` (default 16) sizes the
   front-half publisher queue (measured in _batches_) only for non-group `AllStreams` adapters;
   category adapters and consumer-group members are DB-driven and do not register that queue.
...
<Callout type="warn">
  Direct `kirokuAdapter` users must pass `guardKirokuHandler handler` to `mkProcessor`, or provide an
  equivalent `guardKirokuHandlerWith` mapping. A synchronous exception otherwise prevents Shibuya
  from finalizing the ack and leaves the worker waiting. `kirokuConsumerGroupProcessors` applies the
  default one-second-retry guard automatically; asynchronous cancellation still propagates.
</Callout>

## The Streamly bridge: `subscriptionAckStream`

This is the heart of the back half. The current termination core matters as much as the ack path:

```haskell
-- kiroku-store/src/Kiroku/Store/Subscription/Stream.hs (trimmed)
...
- **Termination is separate from the bounded data queue.** `closedVar` records clean close or worker
  crash. `step` waits on `readTBQueue queue` `orElse` that state: clean close ends the stream, while
  `BridgeCrashed e` rethrows `e` from the next pull. Cancellation can therefore wake an idle reader
  without needing a free queue slot.
...
**The ack `TBQueue AckItem` — bounded, but effectively depth 1.** Its capacity is `bufferSize`
...
generous capacity is therefore not a pipeline depth. A zero capacity is rejected up front with
`InvalidStreamBufferSize`, and shutdown state travels through `closedVar`, not this queue.
**The ack-coupling, not the queue size, is the backpressure.**
...
| shibuya / stream reader (`step`) | data queue `orElse` `closedVar`   | worker `writeTBQueue`, monitor close/crash, or `cancelAction` clean close                                  |
...
With the required handler guard, Shibuya runs to a verdict and calls `finalize`, which is
`tryPutTMVar` and cannot block. The dependency edge is one-way: worker → waits for → Shibuya's
verdict → reply. A direct unguarded handler breaks that premise if it throws synchronously, which is
why `guardKirokuHandler` is part of the adapter contract rather than optional error decoration.
...
- `cancelAction` records `BridgeClosedCleanly` in `closedVar`. A stream reader parked on the
  queue-or-close STM choice wakes, `step` returns `Nothing`, and `unfoldrM` ends the stream. No
  bounded-queue slot is needed. The monitor records the same clean state for a normal stop, but
  records `BridgeCrashed e` for another worker failure so the next pull rethrows it.
...
- **Bounded memory.** The back-half ack queue is depth-1 in practice (ack-coupling), so the bridge
  buffers a single event. The front-half publisher queue is bounded at `queueCapacity` (16 batches)
  for non-group `AllStreams` adapters; the publisher _never blocks_ on a slow subscriber. The adapter
  inherits Kiroku's default `PauseAndResume` policy, so a full publisher queue pauses fan-out, drains
  stale batches, and re-catches-up from the durable checkpoint without dropping events. Category
  adapters and consumer-group members avoid the front-half queue entirely and throttle their own
  DB-driven fetch loops through the same per-event ack.
...
- **No redundant database load, no head-of-line blocking on the shared path.** `NOTIFY` ticks are
  **debounced**: `drainTicks` collapses a burst of notifications into a single fetch. The
  `EventPublisher` reads the `$all` log **once** per cycle (in 1000-event batches) and fans the
  result out to every registered non-group `AllStreams` subscriber, so N `$all` subscribers cost one
  read, not N. And because the publisher decouples from consumers via bounded per-subscriber queues,
  one slow shibuya handler cannot back-pressure the broadcaster and starve the other subscribers;
  under `PauseAndResume` that slow subscriber recovers by re-reading from its checkpoint.
