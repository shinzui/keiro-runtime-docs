...
description: "Reading EventPublisher.hs: the single $all broadcaster that reads new events once and fans them out to every subscriber's bounded queue, with per-subscriber overflow policy."
---
...
broadcaster**. The [worker driver](/docs/kiroku/walkthrough/02-the-worker-driver) reads its
`AllStreams` live events from "the publisher's bounded queue" and treats `lastPublished` as its
catch-up target; this part is the other side of that seam — the single thread that fills those
queues.

The core idea is **read once, fan out many**. Rather than have every `AllStreams` subscription poll
the database for new events independently, one `EventPublisher` thread reads each new batch of the
global stream **once** and hands it to every registered subscriber through that subscriber's own
bounded queue. One database reader, N in-memory consumers.
...
Four functions bound the publisher's life, and they map one-to-one onto the store and a `subscribe`
call:
...
- **`subscribePublisher`** is the registration `subscribe` calls. It is a single STM transaction:
  allocate a bounded `TBQueue` of `cap` batches, a fresh `Active` status, insert the `Subscriber`
  into the registry under a new id, and return the queue, the status `TVar`, and an `unsubscribe`
  action that deletes the entry. **Forgetting to `unsubscribe` is a leak** — the publisher would keep
  pushing to a queue with no reader, which fills and trips the overflow policy needlessly. (That is
  exactly why the worker's `finally` cleanup runs it on every exit — see
  [part 04](/docs/kiroku/walkthrough/04-subscribe-and-lifecycle).)
...
- Every worker — `AllStreams`, `Category`, or group member — uses **`lastPublished`** as its catch-up
  target.
- A **`Category` or consumer-group** worker registers (so it shares `lastPublished` and gets an
  `unsubscribe`) but reads its live events from the database instead, via the NOTIFY-driven or
  DB-driven live loop. Its `subQueue` is therefore "unused for category subscriptions": the publisher
  still fans out to it, but under the default `PauseAndResume` the queue simply fills once and the
  subscriber sits `Paused`, ignored, until it unsubscribes.
...
into a single `$all` read, fans the batch out to every registered subscriber's **bounded** queue with
its own overflow policy applied in-place, and advances `lastPublished` — so N subscriptions share one
database reader and no slow consumer can ever block the broadcast or grow its memory without bound.

## Next

[04 — subscribe & lifecycle](/docs/kiroku/walkthrough/04-subscribe-and-lifecycle): how `subscribe`
registers a subscriber with this publisher, spawns the worker, and hands back a cleanup-guaranteeing
handle.
