...
gapless, strictly monotonic sequence assigned at append time. Every event an `AllStreams` subscription
sees is read from that one ordered spine:
...
- The catch-up→live handoff cannot create a gap or a rewind: catch-up runs until the cursor reaches
  the publisher's position, and the `AllStreams` live path drops anything `≤ cursor` (the `> c`
  filter), so the seam is exactly contiguous.

So _before_ shibuya is even involved, the contract is: **the handler is offered events in strictly
ascending global position, with no gaps and no reordering, even across retries and the live handoff.**
...
`PartitionedInOrder`. The [consumer-group helper](/docs/kiroku/walkthrough/05-consumer-groups-and-policy)
...
- `cancelAction` runs `cancel subHandle` first. If the worker is parked in `takeTMVar` (the common
  ...
  _not_ stuck waiting for a reply that will never come. Its `finally unsubscribe` ([part 03](/docs/kiroku/walkthrough/03-subscribe-and-lifecycle))
