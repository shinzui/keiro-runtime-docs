...
<Callout type="info">
  These docs review `keiki 0.4.0.0` and `keiki-codec-json 0.4.0.0`. Since 0.2 the `Keiki.Decider`
  facade is gone in favour of `SymTransducer` directly; 0.3 added `EdgeMode` (`Live` / `ReplayOnly`)
  and the control-state shape hash, and 0.4 added typed symbolic field projections. See [compatibility and
