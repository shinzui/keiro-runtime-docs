...
<Callout type="info">
  These docs review `keiki 0.8.0.0` and `keiki-codec-json 0.8.0.0`. Since 0.2 the `Keiki.Decider`
  ...
  domains, honest verification verdicts, and detailed step/replay attribution; and 0.8 made
  **readable business semantics the primary rendering contract** (`lit` requires `Show`, `toMermaid`
  shows guards and updates). See [compatibility and
  upgrades](/docs/getting-started/compatibility-and-upgrades) before updating an older application.

  **Note the version skew:** the Keiro pages on this site are still reviewed at Keiro `0.4.0.1`,
  which pins `keiki >=0.4 && <0.5`.
</Callout>
