...
`regFileShapeHash` is a GHC-upgrade-safe discriminator over the register file's structural shape —
its ordered slot list of `(name, type)` pairs. It is sensitive to structural drift (a renamed,
reordered, retyped, added, or removed slot changes the hash) and stable across GHC upgrades that do
not change that shape. This page does **not** re-document how the hash is computed — see the
...
`Keiki.Codec.JSON.GoldenSpec` for the pinned-output golden tests.
...
your back. See `Keiki.Codec.JSON.SensitivitySpec` for the tests that pin both senses down.
