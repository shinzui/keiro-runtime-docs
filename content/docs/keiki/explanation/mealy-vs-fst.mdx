...
The shipped `SymTransducer` (keiki `0.2.0.0`, commit `ce5748b`) takes the **output-widening**
...
the AST, with no internal scaffolding vertices. The retired EP-20 helpers (`toMultiDecider`
and `DriverConfig`) are gone. Forward execution uses `stepEither`, which returns the complete event
list, while hydration uses Core's structured replay functions.
...
`InFlight` wrapper and `applyEventStreamingEither`:
...
`replayEvents` may return an `InFlight` carrier so a paginated reader can resume. The strict
`applyEventsEither` and `reconstituteEither` entry points instead reject a truncated tail with
`ReplayLogTruncated`. Keiki 0.2 deliberately exports no Decider facade: a four-field record cannot
represent both the multi-event carrier and located replay failures without losing information.
