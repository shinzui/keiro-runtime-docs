...
This matters because keiki *derives* a `decide`/`evolve` decider facade and other views from
the same transducer (see [What gets derived](/docs/keiki/reference)). If two edges at one
vertex could both fire on the same input, the runtime would have to pick one and silently drop
the other — and the derived views could disagree about which result is "the" result. Single-
valuedness is the invariant that keeps every derivation in agreement. It is the keiki analogue
