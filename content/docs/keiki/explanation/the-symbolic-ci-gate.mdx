...
`satResultIsProvablyUnsat` recognizes only SBV's definitive `Unsatisfiable` result. `Satisfiable`,
`DeltaSat`, `Unknown`, `ProofError`, timeout, and other failures all return `False`. So one pair of
edges becomes one `sat` call on `g1 AND g2`: `unsat` certifies the pair; every other result means the
pair is overlapping or unresolved.
...
flowchart TB
  ...
  Bot -- "unknown / proof error → isBot = False" --> Uncertain["NOT certified: investigate solver result"]
  Next --> P
  P -- "all pairs OK" --> V
...
When `symIsBot (g1 AND g2)` comes back `False`, the conjunction is either satisfiable or not
certified empty. `isSingleValuedSym` itself only returns a `Bool`; hand the same predicate to
`symSatExt`. A `Just (regs, ci)` is a concrete input that fires both edges. `Nothing` can also mean
solver uncertainty, so do not reinterpret it as proof of disjointness.
...
<Callout type="info">
  **Strengthened in keiki 0.7.0.0.** `symSatExt` now concretely rechecks every reconstructed
  candidate before returning it, so `models p (regs, cmd)` holds **unconditionally** for a `Just`.
  Previously an unrealizable free projection assignment, an opaque `TApp1`/`TApp2` assignment, or a
  non-`Sym` equality fallback could produce a pair that failed concrete evaluation. Those now return
  `Nothing` — which reinforces the rule above: `Nothing` means "no concrete witness recovered", never
  "proved empty".
</Callout>
...
<Callout type="info">
  **When you need to know *why* a gate answered as it did** *(keiki 0.7.0.0)*, the `Bool` is no
  longer the only option. [`verifyPredicate`](/docs/keiki/reference/symbolic) separates "the
  translation was exact" from "the solver was definite", reporting `UnverifiedOpaque`,
  `UnverifiedSolverUnknown`, and `UnverifiedSolverFailure` as distinct outcomes instead of folding
  them into a conservative `False`. `predicateTranslationReport` names the specific reason exactness
  was lost — an opaque application, an unsupported carrier, a projection used outside equality — with
  no solver call at all, and `checkTransitionDeterminismSymDetailed` / `checkDeadEdgesSymDetailed`
  attach that verdict to the exact `EdgeRef` it came from. Useful for turning a red gate into an
  actionable message.
</Callout>
