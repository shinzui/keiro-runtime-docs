...
<Callout type="warn">
  “Projection” in this essay means the mathematical projection of a transducer onto its input or
  output alphabet. It is not an application read-model projection. Keiro read models are built by
  folding stored events and never read the Keiki `RegFile`.
</Callout>
...
keiki (継起, 0.2.0.0) is a pure, IO-free library, and everything above is realized by concrete,
