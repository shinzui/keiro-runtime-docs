...
- `reconstitute` replays a `[co]` event log back into `Maybe (s, RegFile rs)` — the
  event-sourcing read path. See [Deriving event sourcing](/docs/keiki/explanation/deriving-event-sourcing).

<Callout type="info">
  keiki also ships `Keiki.Decider` / `toDecider`, a *legacy compatibility façade* that
  re-presents a `SymTransducer` as the older Decider triple. It is not the recommended
  surface — prefer `step`/`stepEither`/`delta`/`omega`/`reconstitute` for new work. The
  façade is documented separately at `/docs/keiki/reference/decider`.
</Callout>
