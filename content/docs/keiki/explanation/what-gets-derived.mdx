...
description: "How a single SymTransducer declaration fans out into two acceptors, machine-state views, input/wire constructors, and a stable shape hash."
---

In keiki (継起) you author **one** thing — a `SymTransducer`, written with the builder. Keiki derives
several machine artifacts from that declaration. You never re-state your domain in five different
shapes.

<Callout type="warn">
  “Derived artifact” does not mean “event-sourcing projection.” In particular, the generated
  B-presentation `View` reads an already-hydrated `RegFile`; it is unrelated to Keiro projections
  and read models, which fold events.
</Callout>
...
flowchart TD
  ...
  Decl --> Views["B-presentation machine views
  (SVertex GADT + View GADT + RegFile accessor)"]
...
The register file is the complete machine memory (the *C-foundation*). A B-presentation view is a
vertex-indexed accessor that exposes only the slots live at each control vertex. From the vertex
enum plus a per-vertex live-slot spec, `deriveView` emits a singletons GADT, a per-vertex `View`
GADT, and the `RegFile` accessor. It does not consume events, accumulate projection state, or create
a queryable read model. See [B-presentation machine-state
...
The register-file slot list yields `regFileShapeHash` — a discriminator over ordered slot names and
canonical types. Keiki pins built-in names; application types can pin their own persistence identity.
See [Shape](/docs/keiki/reference/shape).
</Step>
...
<Cards>
  ...
  <Card title="B-presentation machine-state views" href="/docs/keiki/explanation/b-presentation-views" />
