---
title: "00 — Start here"
description: "An ordered source tour of keiki's rendering and codec surface: one pure transducer rendered to domain-readable text, Mermaid diagrams, and JSON."
---
...
rendering is shaped the way it is. Read the chapters in order.

## The design in one picture

Every renderer in this surface is **one pure function from a value to `Text` (or JSON)**. There is no
solver and no IO. The same `SymTransducer` feeds every rendering; each renderer reuses the same
...
flowchart LR
  ...
  Comp --> Atlas
  T --> Codec["RegFile snapshots / versioned event envelopes → JSON"]
```
...
<Cards>
  ...
  <Card title="02 — Mermaid core" href="/docs/keiki/walkthrough/rendering-and-codecs/02-mermaid-core" description="edgeLabel, edgeInputName, edgeOutputName, the topology walk, and the readable default." />
  ...
  <Card title="08 — TH derivers" href="/docs/keiki/walkthrough/rendering-and-codecs/08-th-derivers" description="Record snapshot codecs plus pinned, versioned event envelopes and their migration chain." />
  <Card title="09 — Shape hash and snapshots" href="/docs/keiki/walkthrough/rendering-and-codecs/09-shape-hash-and-snapshots" description="Hashing a register slot list and applying the two-discriminant snapshot-compatibility check." />
</Cards>
...
keiki/src/Keiki/Shape.hs            -- register-file shape identity
keiki-codec-json/src/Keiki/Codec/   -- register snapshots and event envelopes
```
