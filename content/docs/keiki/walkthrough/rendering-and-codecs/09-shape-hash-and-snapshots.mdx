...
<Callout type="info">
**The control state is discriminated too.** The two discriminants above cover the **register file**. A
persisted keiro snapshot is a `(s, RegFile rs)` pair, and keiki 0.3.1.0 added
[`stateShapeHash`](/docs/keiki/reference/shape#canonicalstateshape--the-control-state-hash) for the
control-state half — a generic hash over the datatype name, constructor names and order, and field
types. Because that description is purely structural (it cannot see a semantic fold change), keiro
composes it with a caller-owned fold fingerprint, making its own rule a **three**-component
discriminator rather than the two shown here. See [keiro's snapshot
