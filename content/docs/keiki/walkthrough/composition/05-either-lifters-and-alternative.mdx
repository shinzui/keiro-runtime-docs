...
<Callout type="info">
The `Either` wrapping is exactly what makes `alternative` need no cross-transducer mutual-exclusion
check. A `t1`-edge guard requires `Left _` (via `leftInCtor`), a `t2`-edge guard requires `Right _` (via
`rightInCtor`); the two are *structurally* mutually exclusive, so single-valuedness of the composite
reduces to each arm's own check. The `Either` arms make the cross-arm overlap vacuous.
</Callout>
...
the other fixes the alphabet. The target advances `t1`'s vertex and **freezes `s2`** at its incoming
