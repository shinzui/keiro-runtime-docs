...
description: "The jitsurei loan workflow: a process manager turning events into commands, a downstream aggregate, and the three-stage compose that wires them — ending on why the composite is a verification artifact, not a runtime."
---
...
it drives (`Loan`), and the three-stage `compose` that wires the whole chain (`LoanWorkflow`) — then
end on the most important caveat in the entire tour: what the composite *is not*.
...
## The three-stage compose

`jitsurei/src/Jitsurei/LoanWorkflow.hs` wires `LoanApplication`, `CoreBankingSync`, and `Loan` into a
single `SymTransducer`. The seams between contexts are bridged by two `lmapMaybeCi` adapters (chapter
08) — partial contramaps that translate one context's output alphabet into the next context's input
alphabet, returning `Nothing` for the irrelevant cases so those inputs are filtered out.
...
`compose`'s `Disjoint` constraint discharge at each step.

Read the body next. The inner `compose` wires `CoreBankingSync` to `Loan` through
...
one composite firing. `loanWorkflow` is therefore largely a **type-level wiring / verification
artifact** — it proves the alphabets and slot namespaces line up — not a runtime that executes the
flow. The durable, async reaction loop runs through **keiro**, not through the composite.
</Callout>
...
```haskell
-- jitsurei/src/Jitsurei/LoanWorkflow.hs
-- the composite is therefore largely a *type-
