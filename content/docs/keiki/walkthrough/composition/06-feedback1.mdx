---
title: "06 — feedback1"
description: "The single-step two-copy cascade, reduced to two stacked composes, its empty-register constraint, and why it is not shared-state feedback."
---
...
combinator that models a finite *two-copy cascade* through a policy. It is the smallest combinator in
the module — its entire body is a single
...
inner `t` advances independently of the outer `t`; the follow-up command does not return to the state
that processed the external command. This is not shared-state aggregate feedback, and the acceptance
test makes that distinction observable. There is deliberately no `feedback1Checked`: boundary
alignment cannot make the two dimensions share state.
...
```haskell
-- keiki/test/Keiki/CompositionFeedback1Spec.hs
it "reconstituteEither on [cascadedEvent] lands at Composite On (Composite Pol On)" $
  case reconstituteEither loop [cascadedEvent] of
    Right (Composite outerT (Composite policy innerT), _) -> do
      outerT `shouldBe` On
      policy `shouldBe` Pol
      innerT `shouldBe` On
```
...
Each level of nesting adds one more round and another copy of the prior composite. The outer call
therefore requires the first result's entire `Append rs1 (Append rs2 rs1)` register file to be empty:
both the original aggregate and policy must be register-stateless for the shown nested form. For
genuine async, multi-round, durable feedback loops, the composite is a wiring artifact;
