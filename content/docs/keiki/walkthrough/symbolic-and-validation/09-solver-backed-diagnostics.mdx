...
description: "checkTransitionDeterminismSym and checkDeadEdgesSym: the exact, z3-backed determinism and dead-edge checks, and the over- and under-approximation gotchas that make the …Sym variants the only meaningful ones."
---
...
diagnostics. They prove overlaps the pure path cannot, and they come with three gotchas that explain why
...
the exact z3 decision. The Haddock states what this buys:
...
exact, because `isBot` now means "z3 found no model for the conjoined guards."
...
flowchart TB
  ...
  subgraph sym["Solver path (Keiki.Symbolic)"]
    CS["checkTransitionDeterminismSym<br/>= checkTransitionDeterminism . withSymPred<br/>EXACT: z3 isBot"]
  end
  CT -.->|"only meaningful through"| CS
```

The takeaway: the pure path is safe-but-incomplete, the raw polymorphic check is noise on the `HsPred`
carrier, and the exact answer is the polymorphic check *fed the `SymPred` carrier* — which is exactly
what `checkTransitionDeterminismSym` is.
