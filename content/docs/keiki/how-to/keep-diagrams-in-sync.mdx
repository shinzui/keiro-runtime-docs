...
This is exactly how keiki pins its own examples: `toTopologyMermaid loanApplication` is fixed
verbatim in `Jitsurei.Render.MermaidLoanSpec`, and the atlas shape in `Keiki.Render.MermaidSpec`.
Because `toMermaidWith defaultMermaidOptions t` is byte-identical to `toMermaid t`, you can pin any
policy the same way once you settle on a `MermaidOptions` value.

<Callout type="info">
Pin the **render call you actually publish**. If your doc shows the compact topology
(`toTopologyMermaid t`) or a custom policy (`toMermaidWith opts t`), pin *that* call — not the bare
`toMermaid t` — or the golden and the doc will disagree.
</Callout>

<Callout type="warn">
  **Upgrading from keiki 0.7 will fail every diagram golden.** `toMermaid` and every no-options
  shape renderer now emit readable guards, complete register assignments, and multiline labels. Two
  honest fixes: regenerate the expectation (§2 below) if the doc should show behaviour, or switch
  the call to `toTopologyMermaid` if it was always meant to be a topology skeleton. Do not reach for
  `showWrittenSlots` / `showGuardSummary` — those fields were removed.
</Callout>
