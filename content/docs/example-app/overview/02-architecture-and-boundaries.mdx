...
description: "The two-service topology of keiro-runtime-jitsurei and why the services share no service runtime implementation — only versioned messages over Kafka plus an internal pgmq queue."
---

`keiro-runtime-jitsurei` is deliberately built as **two independent services that share no service runtime
implementation**. Each owns its own database, its own Kiroku event store, its own read models, its own
...
- **Honest boundaries.** Sharing Haskell types makes it tempting to share *more* — a domain type, a
  helper, an enum — and erode the boundary. With separate service implementation, the only contract is the message
  on the wire.
...
| Read models | `incident_dashboard`, `field_resource_assignments`, `pending_transfer_needs`, `evacuation_zones`, `command_audit_history`, capacity-change candidates | `hospital_readiness`, `transfer_decisions`, transfer candidates, accepted transfer needs |
...
into its own inbox with durable deduplication. The [cross-service](/docs/example-app/cross-service) tour follows a
single message all the way across this topology.

## Component ownership

Project `mori://shinzui/keiro-runtime-jitsurei` composes one embedded migration plan per service.
Kiroku owns `kiroku`, Keiro owns `keiro`, PGMQ owns `pgmq`, and the applications own
`incident_command` and `hospital_capacity`. The project-relative contract schema and fixtures
named above live under `docs/contracts/` (artifact-level URIs pending); they are also indexed by
`mori://shinzui/keiro-runtime-jitsurei/docs/integration-contracts`.
