...
description: "The headline end-to-end story: a critical patient travels from triage to a confirmed bed and back, with durable evidence across two services."
---
...
The canonical narrative is in `mori://shinzui/keiro-runtime-jitsurei/docs/scenarios`,
project-relative `docs/scenarios/mass-casualty-transfer.md` (artifact-level URI pending).
...
sequenceDiagram
    ...
    PQ->>HC: runJobOnceWithContext → RequestTransferReservation (hold bed)
...
4. **Hospital Capacity deduplicates delivery.** Its Kafka consumer hands the envelope to the
...
6. **A job drains and holds a bed.** `runJobOnceWithContext` decodes the item and runs
...
Kafka producer/consumer parentage is verified, while the pinned one-shot PGMQ worker has a
separate continuity boundary. See [telemetry and trace continuity](/docs/example-app/cross-service/04-telemetry-and-trace-continuity).
...
| Reservation work leased and acknowledged | Keiro PGMQ one-shot job runtime |
| Application, Kafka-parentage, and PGMQ operation evidence | OpenTelemetry |
