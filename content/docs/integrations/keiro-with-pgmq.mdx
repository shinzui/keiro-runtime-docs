...
- **`hospital-capacity`** — one reservation-work queue **drained one-shot** directly from PGMQ with
  `runJobOnce` on an external cadence, using a literal `JobCodec` (to preserve its exact wire format)
  and a DLQ policy. The context-aware drain returns how many messages it disposed, which is useful for
  scheduled workers and tests.
