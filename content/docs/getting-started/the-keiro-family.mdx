...
`keiro-ops` is the operational console — a standalone database-only binary and an
embeddable command library for inspecting and repairing a running deployment.
Every command is a thin adapter over an exported operation from the library that
owns the affected state, so it never reproduces a supported mutation with ad hoc
SQL. Like `keiro-dsl`, it is not required by a running service. *New in Keiro
0.12.0.0.* See the [keiro-ops CLI](/docs/keiro/reference/keiro-ops-cli).
