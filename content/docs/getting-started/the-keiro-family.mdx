...
The keiro repository also ships **`keiro-dsl`**, an *authoring* package and CLI (not a runtime
dependency). A `.keiro` file is a typed **specification** of a keiro service; `keiro-dsl` checks it,
scaffolds the symbol-free deterministic layer (domain ADTs, codecs, stream/projection wiring) plus
typed holes for the behaviour-bearing code, and gates schema evolution. The code it emits depends only
on keiro/keiki. See [The keiro-dsl service toolchain](/docs/keiro/explanation/the-keiro-dsl-toolchain).
